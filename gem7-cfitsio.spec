...
# IOC does `ld < .../cfitsio/V4-1/bin/ppc604/cfitsiolib` at boot, so the copy on
...
# "gem7-cfitsio" could not satisfy that -- dnf would delete V1-9-4 to install
...
%global supdir  /gemini/epics3.13.4/support/cfitsio
...
%global expected_name gem7-cfitsio-%{libdir}
...
Summary:        CFITSIO %{version} for GEM7, cross-compiled for vxWorks ppc604
License:        NASA HEASARC (public domain) / Gemini packaging
...
Provides:       gem7-cfitsio = %{version}

%description
CFITSIO %{version}, the version the GEM7 IOCs load at boot, installed at
...
%package devel
Summary:        Headers for building against CFITSIO %{version}
Provides:       gem7-cfitsio-devel = %{version}
%description devel
...
# cfitsioInstall passes -T ppc604 -T mv167; only ppc604 is used by any
# current GEM7 IOC and mv167 doubles the build, so it is dropped here.
...
for f in bin/ppc604/cfitsiolib; do
...
%changelog
* Mon Sep 14 2026 Hawi Stecher <hawi.stecher@noirlab.edu> - 4.1-1
- First packaged build. Source is CVS tag V4-1 from
